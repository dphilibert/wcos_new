...
function createImage($config, $formulaPath, $imagePath) {
	if (is_file($formulaPath) && ($handle = fopen($formulaPath, 'r')) !== false) {
		...
		file_put_contents($imagePath, $response);
                ...
                // copy image to frontend server
                $path = '/var/www/vhosts/typo3.elektroniknet.de/httpdocs/fileadmin/user_upload/wor_forms/';
                $command = 'scp '.$imagePath.' root@typo3.elektroniknet.de:'.$path.basename($imagePath);
                exec($command);
