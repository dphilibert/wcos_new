<?php
  ...
  /**
   * Datenbank-Model für allgemeine Daten z.B. Statistik
   *
  ...
  class Model_DbTable_GeneralData extends Zend_Db_Table_Abstract
  {
    ...
    public function saveVisit ($anbieterID)
    {
      $visitTime = mktime ();
      $db = Zend_Registry::get ('db');
      $sql = "INSERT INTO stats_visits (vmKundennummer, visitTimestamp) values ($anbieterID, '$visitTime')";
      try
      {
        $db->query ($sql);
      } catch (Zend_Exception $e)
      {
        logError (print_r ($e->getMessage(), true), "insert error");
      }
    }
